// @ts-nocheck
// Scream Tracker 3 parser and mixer (ESM, strict-safe)
function dos2utf(c) { return String.fromCharCode(c); }
var i, j, t, s, ch, outp, fl, fr, fs, f, p, pp, c, row, offset, smpoffset, delta;

var defaultEmptySample = {
  length: 0,
  loopstart: 0,
  loopend: 0,
  looplength: 0,
...
function getSample(mod, idx) {
  if (mod && mod.sample && mod.sample[idx]) {
    return mod.sample[idx];
  }
  return defaultEmptySample;
}
...
function Screamtracker()
{
  ...
  for(var i=0;i<32;i++) { this.pan_r[i]=0.5; this.pan_l[i]=0.5; }
...
Screamtracker.prototype.clearsong = function()
{
  ...
  this.sample = new Array(256);
  for(var i=0; i<256; i++) {
    this.sample[i] = createEmptySample();
  }
...
Screamtracker.prototype.parse = function(buffer)
{
  ...
  for(var i=0;i<4;i++) this.signature+=String.fromCharCode(buffer[0x002c+i]);
  ...
  // load orders
  for(var i=0;i<this.ordNum;i++) this.patterntable[i]=buffer[0x0060+i];
  ...
  for(var i=0;i<this.insNum;i++) {
    this.sample[i]=new Object();

    var offset=(buffer[0x0060 + this.ordNum + i*2]|buffer[0x0060 + this.ordNum + i*2 + 1]<<8)*16, j=0;
    this.sample[i].name="";
    ...
    for(var j=0;j<this.sample[i].length;j++) this.sample[i].data[j]=(buffer[smpoffset+j]-128)/128.0; // convert to mono float signed
  }
  ...
  for(var i=0;i<this.patNum;i++) {
    ...
    for(var row=0;row<64;row++) for(var ch=0;ch<this.channels;ch++) {
    ...
    while(row<64) {
      if (c=buffer[offset+pos++]) {
        ...
        if (ch<this.channels) {
          if (ch>max_ch) {
            for(var j=0;j<this.songlen;j++) if (this.patterntable[j]==i)
  ...
  for(var i=0;i<this.channels;i++) this.chvu[i]=0.0;
...
Screamtracker.prototype.process_note = function(mod, p, ch) {
  ...
  if (s) {
    ...
    var samp_s = getSample(mod, s-1); mod.channel[ch].volume=samp_s.volume;
    mod.channel[ch].voicevolume=mod.channel[ch].volume;
    if (n==255 && (mod.channel[ch].samplepos > samp_s.length)) {
  ...
  if (n<254) {
    ...
    n=(n&0x0f) + (n>>4)*12;
    var samp_ch = getSample(mod, mod.channel[ch].sample); pv=(8363.0 * mod.periodtable[n]) / (samp_ch.c2spd || 8363);
...
Screamtracker.prototype.process_tick = function(mod) {
  ...
  for(var ch=0;ch<mod.channels;ch++) {
    ...
    // kill empty samples
    var samp_tick = getSample(mod, mod.channel[ch].sample); if (!samp_tick || !samp_tick.length) mod.channel[ch].noteon=0;
  ...
  for(var s=0;s<buflen;s++)
  {
    ...
    for(var ch=0;ch<mod.channels;ch++)
    {
      ...
      if (mod.channel[ch].noteon || (!mod.channel[ch].noteon && mod.channel[ch].volramp<1.0)) {
        var curSamp = getSample(mod, si); if (curSamp.length > mod.channel[ch].samplepos) {
          fl=mod.channel[ch].lastsample;

          // interpolate towards current sample
          var f=mod.channel[ch].samplepos-Math.floor(mod.channel[ch].samplepos), fs=(curSamp.data && curSamp.data[Math.floor(mod.channel[ch].samplepos)]) || 0.0;
          fl=f*fs + (1.0-f)*fl;
        ...
        // loop or stop sample?
        var loopSamp = getSample(mod, mod.channel[ch].sample);
        if (loopSamp.loop) {
          if (mod.channel[ch].samplepos >= loopSamp.loopend) {
            mod.channel[ch].samplepos -= loopSamp.looplength;
            mod.channel[ch].lastsample = mod.channel[ch].currentsample;
          }
        } else if (mod.channel[ch].samplepos >= loopSamp.length) mod.channel[ch].noteon = 0;
      }
...
Screamtracker.prototype.effect_t0_o=function(mod, ch) { // set sample offset
  ...
  var effSamp = getSample(mod, mod.channel[ch].sample); if (mod.channel[ch].lastoffset*256 < effSamp.length) {
